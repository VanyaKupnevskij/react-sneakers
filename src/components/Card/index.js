...
{    
    const { onClickBuy, onClickFavorite } = useContext(AppContext);

    return (
        <div className={styles.card}>
            <button className={`${styles.favorite} ${(cardInfo.isFavorite ? styles.liked : "")}`} 
                onClick={() => onClickFavorite(cardInfo, cardInfo.isFavorite)}>
                <svg className={styles.favorite_img} width="15" height="14" viewBox="0 0 15 14" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className={styles.buy}>
                ...
                <button className={`${styles.buy_btn} ${(cardInfo.inBasket ? styles.clicked : "")}`} 
                    onClick={() => onClickBuy(cardInfo, cardInfo.inBasket)}>
